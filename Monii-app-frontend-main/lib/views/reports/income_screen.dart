...
class _IncomeScreenState extends State<IncomeScreen> {
  ...
  Future<void> fetchIncomeData() async {
    try {
      ...
      final response = await http.get(
        Uri.parse('http://10.0.2.2:8080/api/analytics/category-summary?type=1'),
        headers: {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          incomeCategories = data.map((item) {
            return {
              "categoryId": item['categoryId'], // ✅ Ensure ID is included
      ...
      debugPrint('Error fetching income data: $error');
  ...
  Widget _buildPieChart() {
    final total = incomeCategories.fold(0.0, (sum, item) => sum + item['total']);
    ...
    return AspectRatio(
      ...
      child: PieChart(
        PieChartData(
          ...
          sections: incomeCategories.map((item) {
  ...
  Color _getCategoryColor(String categoryName) {
    ...
    final color = Color.fromARGB(
      255,
      100 + rand.nextInt(155), // Avoid too-dark colors
